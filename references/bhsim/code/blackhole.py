...
def generate_image_data(
    ...
    """Generate the data needed to produce an image of a black hole and optionally save it.
    ...
    theta_0 : float
        Inclination of the observer, in radians, with respect to the the normal of the accretion disk.
    n_vals : Iterable[int]
        Order of the calculation; n=0 corresponds to the direct image, n>0 are ghost images.
    m : float
        Mass of the black hole.
    ...
    save_path : str, optional
        The path where the DataFrame will be saved. If None, the DataFrame is not saved.
    file_format : str, optional
        The file format to save the DataFrame ('csv' or 'excel'). Default is 'csv'.
    ...
    for n in n_vals:
        with mp.Pool(mp.cpu_count()) as pool:
            ...
            for r, (alpha_reoriented, b, opz_vals, flux) in zip(
                r_vals, pool.starmap(simulate_flux, args)
            ):
                a_arrs.append(alpha_reoriented)
                b_arrs.append(b)
                opz_arrs.append(opz_vals)
