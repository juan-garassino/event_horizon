...
def generate_isoradials(
    ...
    for n in sorted(n_vals)[::-1]:
        for r in r_vals:
            ...
            iso = Isoradial(
                blackhole.reorient_alpha(alpha, n),
                blackhole.impact_parameter(alpha, r, theta_0, n=n, m=1),
                r,
                theta_0,
                n,
            )
...
def generate_scatter_image(
    ...
    """
    theta_0 = th0 * np.pi / 180
    df = blackhole.generate_image_data(alpha, r_vals, theta_0, n_vals, m, {"max_steps": 3})
